<?php 
	...
	if(!isset($_SESSION['room']))
	{
		...
		$sql = "CREATE TABLE `user`.`$room` ( `sn` INT(128) NOT NULL AUTO_INCREMENT ,`status` INT(1) DEFAULT 1, `username` VARCHAR(128) NOT NULL , `dp` VARCHAR(128) NOT NULL,  `msg` VARCHAR(128) NOT NULL , `date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`sn`)) ENGINE = InnoDB;";
	...
	if(isset($_GET['id']))
	{
		$id = $_GET['id'];
		// $sql = "CREATE TABLE `epiz_27865341_user`.`$id` ( `sn` INT(128) NOT NULL AUTO_INCREMENT , `username` VARCHAR(128) NOT NULL , `dp` VARCHAR(128) NOT NULL, `msg` VARCHAR(128) NOT NULL , `date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`sn`)) ENGINE = MyISAM;";
		$sql = "CREATE TABLE `user`.`$id` ( `sn` INT(128) NOT NULL AUTO_INCREMENT ,`status` INT(1) DEFAULT 1, `username` VARCHAR(128) NOT NULL , `dp` VARCHAR(128) NOT NULL,  `msg` VARCHAR(128) NOT NULL , `date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`sn`)) ENGINE = InnoDB;";
		$result = mysqli_query($conn, $sql);
		if($result)
		{
			$_SESSION['room'] = $id;
...
<div id="frame">
	<div id="sidepanel">
		...
		<div id="contacts">
			...
			<!-- // Automatic load friends data using ajax from hatInput.php -->
	...
	<div class="content">
		...
		<div class="messages">
			<ul id="autodata2">
				...
				// Automatic load chat data using ajax from hatInput2.php
